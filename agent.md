...
- WSL install should target Debian explicitly with `wsl --install --distribution Debian --no-launch` and should ensure `%UserProfile%\.wslconfig` contains `[wsl2]`, `networkingMode=mirrored`, and `vmIdleTimeout=86400000` without duplicating settings or removing unrelated config. It should also create/update the hidden `WindowsDebloater WSL Debian Keepalive` startup scheduled task for the current Windows user using `wsl.exe -d Debian --exec /bin/bash -lc "exec sleep infinity"` with no execution time limit.
